...
WITH unique_comment_dates AS (
    ...
    FROM {{ ref('stg_youtube_videos_comments') }} -- Reference your staging comments table
    WHERE comment_created_at IS NOT NULL
),

-- Union with unique dates from the posts staging table
unique_video_dates AS (
    SELECT 
        DISTINCT
        DATE(video_created_at) AS date_actual
    FROM {{ ref('stg_youtube_videos') }} -- Reference your staging posts table
